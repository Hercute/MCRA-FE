...
const RecipeOrder: React.FC = () => {
  ...
  const handleDescription = (id: number, value: string) => {
    setRecipePage((prevPages) => prevPages.map((page) => (page.id === id ? { ...page, description: value } : page)));
  };
  return (
      ...
      {recipePage.map((page, index) => (
        <div className="orderItem" key={page.id}>
          <div className="orderFileBox" onClick={handleDivClick}>
            {filePreview ? (
              <Image src={filePreview} alt={page.description} className="uploadedImage" width={80} height={80} />
            ) : (
